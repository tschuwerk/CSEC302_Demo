...
userRoutes.route('/login-fixed').post(function(req, res) {
...
userRoutes.route('/lookupfixed').get(function(req, res) {
...
userRoutes.route('/lookupfixed').post(function(req, res) {
        let username = String(req.body.username);
        
	function isAlphanumeric(str) {
                return str.match(/^[a-zA-Z0-9]+$/) !== null;
        }
	sanitize = isAlphanumeric(username);
        if (isAlphanumeric(username) == true) {
		console.log("request " + JSON.stringify(username));
        	if (typeof username !== 'undefined') {
                	query = { $where: `this.username == '${username}'` }
